...
func Media(w http.ResponseWriter, r *http.Request) {
	...
	if ok, _ := Authenticated(r); ok && r.Method == "POST" {
		...
		GetEngine().Table("media").Select("media.*,subtitle.*").
			Join("INNER", "subtitle", "subtitle.id = media.subtitleid ").Where("subtitleid = ?", vars["id"]).Find(&users)

		result.Medias = users
		OpenTemplate(w, r, result, "AddMedia.html", Models.HeaderVariables{Title: "Medias"})
		...
		OpenTemplate(w, r, result, "AddMedia.html", Models.HeaderVariables{Title: "Media"})
	} else {
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
}
